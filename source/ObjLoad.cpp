...
//#include <pair>
#define DBG_LOG(s, ...) fprintf(stderr, "%s:%d - DBG: "  s "\n" , __FILE__, __LINE__, ##__VA_ARGS__)
#define ERR_LOG(s, ...) fprintf(stderr, "%s:%d - ERR: "  s "\n" , __FILE__, __LINE__, ##__VA_ARGS__)
...
static int ProcessReloc(ObjHandle obj, Elf64_Rel *relocs, size_t sz, uintptr_t sectLoadAddr){
    
    return -1;
}

static int ProcessReloca(ObjHandle obj, Elf64_Rela *relocs, size_t sz, uintptr_t sectLoadAddr){
    ...
    for(size_t i = 0; i < count; ++i){
        ...
        uintptr_t relocValue = reinterpret_cast<uintptr_t>(symAddr) + reloc.r_addend;

        switch(ELF64_R_TYPE(reloc.r_info)){
            case R_X86_64_PC32:
            case R_X86_64_PLT32: // Im pretty sure for object files this is the same as above
            {
               //TODO need to check for overflow
               *reinterpret_cast<uint32_t*>(patchPoint) = static_cast<uint32_t>(relocValue - patchPoint);
            }
            break;
            default:
...
int ProcessRelocs(ObjHandle obj, Elf64_Shdr *reloc){
    ...
    switch(reloc->sh_type){
        case SHT_REL:
            rc = ProcessReloc(obj, static_cast<Elf64_Rel*>(sectionData), reloc->sh_size, sectionLoadAddr);
            break;
        case SHT_RELA:
            rc = ProcessReloca(obj, static_cast<Elf64_Rela*>(sectionData), reloc->sh_size, sectionLoadAddr);
            break;
        default:
...
int ProcessSymbolTable(ObjHandle obj, Elf64_Shdr *symSection, Elf64_Shdr *strInfo){
    ...
    for(int i = 0; i < num_symbols; ++i){
        ...
        if(symbol.st_shndx == SHN_UNDEF){
            ...
            if(symbolAddr == NULL){
                DBG_LOG("Couldnt find symbol %s in objects, looking in native tables", symbolName);
                symbolAddr = dlsym(RTLD_DEFAULT, symbolName);
            }
            if(NULL == symbolAddr){
                DBG_LOG("Failed to resolve symbol %s", symbolName);
                // screw it, lets just pretend everything is fine and symbol address is NULL
                symbolAddr = NULL;
            }
            ...
            void * const symbolAddr = static_cast<char*>(sectionLoad->second) + symbol.st_value;
...
ObjHandle LoadElf(const Elf64_Ehdr& header, FILE *f){
    ...
    //get the shstrtab
    if(ReadStringTable(object, 
        reinterpret_cast<Elf64_Shdr*>(static_cast<char*>(secthdrs) + (header.e_shstrndx * header.e_shentsize)))){
        ...
        switch(shdr->sh_type){
            ...
            case SHT_SYMTAB:
            {
                ...
                Elf64_Shdr *strTable = reinterpret_cast<Elf64_Shdr*>(
                    static_cast<char*>(secthdrs) + (shdr->sh_link * header.e_shentsize));
    ...
    for(auto it = RelocationSections.begin(); it != RelocationSections.end(); ++it){
        Elf64_Shdr *shdr = reinterpret_cast<Elf64_Shdr*>(
...
static int InitCache(){
    if(codeCache == NULL){
        codeCache = mmap(NULL, CACHE_SIZE, PROT_READ|PROT_WRITE|PROT_EXEC, MAP_ANONYMOUS|MAP_PRIVATE, -1, 0);
    ...
    if(dataCache == NULL){
        dataCache = mmap(NULL, CACHE_SIZE, PROT_READ|PROT_WRITE|PROT_EXEC, MAP_ANONYMOUS|MAP_PRIVATE, -1, 0);
...
#define CHECK_IDENT(n) (header.e_ident[EI_MAG##n] ==  ELFMAG##n)
    if(! (CHECK_IDENT(0) && CHECK_IDENT(1) && CHECK_IDENT(2)) ){
        ERR_LOG("Bad magic %x %x %x", header.e_ident[0], header.e_ident[1], header.e_ident[2]);
        goto end;
    }
#undef CHECK_IDENT
